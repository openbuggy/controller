...
static const float BATTERY_A_FACTOR = 0.00117307136f;
static const float BATTERY_B_FACTOR = 0.00111691884f;
static const float BATTERY_MEASURE_FREQ = 1;
static const float BATTERY_SEND_INTERVALL_S = 1;

static const float PWM_MS_LEVEL = CPU_FREQ / PWM_CLKDIV / 1000.f;
static const uint16_t MAX_COUNTER_WITHOUT_UPDATE =
    MAX_TIME_WITHOUT_UPDATE_MS / 1000.f * PWM_SET_LEVEL_FREQ;
static const uint16_t BATTERY_SEND_COUNTER =
    BATTERY_SEND_INTERVALL_S * BATTERY_MEASURE_FREQ;
...
bool measure_battery_callback(struct repeating_timer* t) {
  ...
  if (measure_battery_counter == BATTERY_SEND_COUNTER) {
    const uint16_t buffer[] = {(uint16_t)(voltage_a * 100),
                               (uint16_t)(voltage_b * 100)};
...
int main() {
  ...
  pwm_set_both_levels(
      PWM_SLICE, PWM_MS_LEVEL + (uint16_t)(NEUTRAL_THROTTLE * PWM_MS_LEVEL),
      PWM_MS_LEVEL + (uint16_t)(NEUTRAL_STEERING * PWM_MS_LEVEL));

  struct repeating_timer timer;
  add_repeating_timer_ms(-1000 / PWM_SET_LEVEL_FREQ, set_levels_callback, NULL,
                         &timer);
  ...
  add_repeating_timer_ms(-1000 / BATTERY_MEASURE_FREQ, measure_battery_callback,
                         NULL, &battery_timer);
